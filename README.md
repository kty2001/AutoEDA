...
> **상태: 배포됨 — <https://autoeda.tyoujungzz.workers.dev>.** CSV 읽기부터 통계·품질 점수·발견 목록·차트·전처리·타깃 분석·차원축소·결과 공유까지 구현되어 있고 테스트 368건이 통과함.
> 해설 23편 · 사례 리포트 3편 · 용어집이 발행되어(색인 30 URL) 죽은 내부 링크는 0건이고, 콘텐츠 게이트(15편·25,000자)를 28편·48,125자로 넘겼음. 필수 페이지 4종도 확정했음.
> **다만 프로덕션은 2026-08-18 배포본임** — 그 뒤의 전처리·타깃·공유 링크·차원축소는 아직 배포되지 않았고 브라우저 실기 검증도 남아 있음([docs/TODO.md](docs/TODO.md) T8·T9).
...
| 전처리 | 발견 → 조치 매핑, 연산 7종(중복·열 제거·대치·이상치·로그·인코딩·스케일링), Before/After 비교, 정제 CSV 내보내기 |
| 타깃 | 타깃 열 지정 시 회귀/분류 자동 판별 — 타깃 분포·왜도, 피처 상관 순위, 클래스 분포·클래스별 수치 요약 |
| 차원축소 | 표준화(상관행렬) 주성분 분석 — 스크리, 누적 설명 분산, 성분별 로딩 |

앞의 5개가 Phase 1, 전처리·타깃은 T7, 차원축소는 T9 산출물임. 정보 과부하 방지를 위해 모든 그래프를 생성하지 않고, 통계적 특성으로 선별한 대표 시각화만 제시함.
...
- **Phase 1.5** — 해설 문서 및 사례 리포트 발행, Finding ↔ 해설 링크 연결, sitemap·색인 · *발행 완료 — **해설 23편 · 사례 3편 · 용어집**, Phase 1에서 발화하는 Finding 전량에 해설 연결. 색인은 GSC 등록 대기*
- **Phase 2** — ~~타깃 기반 EDA~~ · ~~내보내기·공유~~ *(완료)* · 단계별 가이드 UI, AI 해석 레이어(BYO API Key), 비동기 처리, 대용량 샘플링
- **Phase 3 (후보)** — AI 질의응답, 데이터셋 비교, ~~전처리 지원~~ *(T7 로 앞당겨 완료)*, 시계열 분석, 분석 이력 비교, 콘텐츠 확장
...
| [docs/DESIGN.md](docs/DESIGN.md) | 색·간격·서체 토큰, 컴포넌트 규격, 화면 구조(히어로 분할·feature band·플로팅 CTA) |
...
```bash
npm test            # 전체 테스트 368건 (계약·동작·통합·빌드·산출물 폐합)
