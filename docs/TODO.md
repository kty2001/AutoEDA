...
### T12. 차트 크기 조정 + 사용 방법·FAQ 페이지 + 랜딩 로드맵 — **완료 (2026-09-24)**

별도 사용자 요청 5건. 원격 T11(차원축소)과 병렬로 진행되어 병합 시 번호를 T12 로 옮김. 차원축소 탭 추가에 맞춰 사용 방법·FAQ·랜딩 EDA 로드맵에도 차원축소 단계를 넣음.

1. **가로 막대 — 범주가 적을 때 막대가 위아래 끝으로 벌어짐** — 행 높이 상한(`BAR_ROW_MAX` = 24 ÷ 0.7)을 두고 묶음을 플롯 세로 중앙에 배치. 범주 10개처럼 행이 이미 작으면 좌표 불변
2. **박스플롯 축소** — viewBox 300 → 240, CSS 상한 300px → 240px. 실기 중 **기존 결함 발견** — `.column-grid .chart { max-width: 100% }` 가 같은 특정도로 뒤에 있어 박스플롯 상한을 덮어 칸 폭 전체(≈570px)로 그려지고 있었음. 박스플롯 규칙을 그 뒤로 옮겨 해소
3. **상단 네비에 '문의'** — 수기 페이지 8종 + `build_guides.mjs` 셸 + 신규 2페이지. 네비 일치 테스트 기대값 갱신
4. **`/pages/how-to` · `/pages/faq` 신설** — 색인 대상, `build_seo.mjs PAGES` · `menu.js ITEMS` · HTML 사이트맵 '안내'에 등록. 명세는 [`screens.md` §3.7.1](screens.md)
5. **랜딩 로드맵** — '무엇을 진단하나' 앞에 ML 파이프라인 8단계(데이터 이해·전처리를 `AutoEDA` 배지로 강조) + EDA 6단계(결과 탭 대응) 펼침. SVG 가 아니라 `<ol class="roadmap">` + CSS — 폭에 따라 줄바꿈되고 640px 이하에서 세로로 쌓임
