...
이 문서는 리테일 브랜드에서 추출한 것이라 한국어 문서 사이트인 AutoEDA 에 그대로 옮기면
깨지는 항목이 있음. 아래는 **의도적으로 다르게 구현한 것**이며 근거는 `css/style.css` 주석에도 남아 있음.

| 항목 | 원문 | AutoEDA 구현 | 이유 |
|---|---|---|---|
| 루트 폰트 크기 | `62.5%` (1rem = 10px) | `100%` 유지, rem 은 px 환산 | 기존 rem 값 전량 재계산이 필요하고 사용자 글자 크기 설정과 어긋남 |
| 자간 | `-0.01em` 전역 | 제목·버튼·탭·배지 등 Latin 비중이 높은 UI 에만 | 음절 폭이 일정한 한글에서 음수 자간은 가독성을 떨어뜨림. 본문 행간 1.7 도 유지 |
| h1·h2 | 같은 크기(24px), 무게·색으로만 구분 | 크기 차이를 남기고 색·무게 규칙만 따름 | 해설 문서는 제목 단계가 3층이라 크기 단서가 없으면 구조가 읽히지 않음 |
| 버튼 여백 | `7px 16px` (높이 32px) | `12px 24px` | §8 이 스스로 지적한 터치 최소치 미달을 해소 |
| Gold | Rewards 전반 | 품질 점수 '양호' 표시 **한 곳**, 글자색이 아닌 테두리로만 | AutoEDA 에 Rewards 개념이 없음. `#cba258` 은 흰 배경 대비 2.1:1 이라 글자로 쓸 수 없음 |
...
| 상관 히트맵 | 규격 없음 | Green Accent(+) ↔ Red(−), 중립점은 Ceramic | 발산 배색은 두 극 + 무채 중립점이 필요함. 색각 이상 분리도 검증함(deutan ΔE 9.9 > 8), 각 셀에 수치를 직접 표기하고 `<title>` 로도 동반 |

**브랜드 경계:** 색·형태 언어만 차용하고 Starbucks 워드마크·로고·사이렌은 쓰지 않음.
`Frap` 같은 고유 명칭도 코드·클래스명에 넣지 않음.
...
The primary Korean typeface is `"Noto Sans KR"`.

Since SoDoSans does not provide Korean glyphs, Korean text must use Noto Sans KR while Latin characters continue to use SoDoSans.
