...
| 쿼리스트링 미사용 (예외 1건) | 도구가 단일 페이지 4상태이므로 상태를 URL로 구분하지 않음 (§4). **유일한 예외**: `/pages/analyze?s=…` — 결과 요약 공유 링크(T8, [`data-model.md §3.8`](./data-model.md)). 4상태 자체는 여전히 URL과 무관하며, 이 파라미터는 외부에서 들어오는 공유 진입점 용도로만 씀. 정상 진입 후에는 `history.replaceState`로 다시 지워 예외가 커지지 않게 함 |
...
| `/pages/case/{slug}` | 사례 리포트 3편(계획 6편) | 빌드(`build_guides.mjs`) | ○ | ○ | ○ |
| `/pages/glossary` | 용어집 — 8절 51항목 | 빌드 | ○ | ○ | ○ |
| `/pages/how-to` | 사용 방법 — 결과 탭 순서대로 도구 사용 절차 (2026-09-24 추가) | 수기 | ○ | ○ | ○ |
| `/pages/faq` | FAQ — 주제별 문답 모음 (2026-09-24 추가) | 수기 | ○ | ○ | ○ |
...
| `/pages/disclaimer` | 면책조항 (2026-09-06 추가) | 수기 | `noindex,follow` | × | × |
| `/pages/sitemap` | HTML 사이트맵 (2026-09-06 추가) | 빌드 | `noindex,follow` | × | × |
| `/404` | 404 | 수기 | × | × | × |

### 설계 결정 3건

1. **`/pages/guide` 인덱스에 K1 본문을 합침** — 목록만 있는 인덱스는 thin content가 되어 안티패턴 #3에 걸림. `content-strategy.md §2.5`의 K1(EDA 진행 순서 체크리스트)이 이미 hub-and-spoke의 허브 문서이므로 이를 인덱스 본문으로 삼음. 결과적으로 해설 23편 = **인덱스 1편(K1) + 하위 22편**임
...
| 화면 구성 | **히어로 40/60 분할**(좌: 한 줄 소개 + `내 데이터 분석하기` CTA / 우: 결과 미리보기 카드) / 분석 항목 요약 / **전면 폭 feature band** / 해설·사례 링크 / FAQ / 푸터 |
...
### 3.6 `/pages/glossary` 용어집

| 항목 | 내용 |
|---|---|
| 목적 | 결과 화면에 나오는 용어 하나를 확인하게 함. 해설 한 편을 읽지 않고도 판정 기준까지 닿음 |
| 진입 경로 | 검색 유입, 상단 네비, 전역 메뉴, 해설 페이지의 관련 절 |
| 담당 UC | UC-25 |
| **초기 HTML 필수** | 용어 40여 항목의 정의 전문. 런타임 fetch 없음 |
| 화면 구성 | 한 줄 요약 / 빠른 이동(본문 앵커에서 자동 생성) / 군별 용어 항목(`h3` + 정의 문단 + 관련 해설 링크) / 임계값 읽는 법 / `내 데이터로 확인하기` CTA |
| JS 모듈 | `app/common.js` |
| 이탈 경로 | 관련 해설, `/pages/analyze`(CTA) |
| 비고 | 항목마다 이 도구가 쓰는 판정 수치를 함께 적음. 임계값의 단일 원천은 [`rules.md §2·§3`](./rules.md)이며 여기서 값을 재정의하지 않음. 해설·Finding 문구를 복사하지 않음 — 안티패턴 #4 |
...
| `/pages/disclaimer` | `terms` §2 의 면책을 보충 — 전문 자문 대체 아님, 정확성 무보증, 판정 기준의 성격, 외부 데이터셋 면책 | — |

5종 모두 `noindex, follow` + sitemap 제외. 푸터 링크로만 도달함.

> `privacy`의 삭제 방법 기재는 UC-11(결과 폐기)이 실제로 구현되어야 성립함. **문구와 구현의 불일치를 만들지 않음.**

### 3.7.1 `/pages/how-to` · `/pages/faq` (2026-09-24 추가)

둘 다 수기 HTML이며 색인 대상(sitemap 포함, CTA 는 how-to 만). 진입 경로는 전역 메뉴 · HTML 사이트맵 '안내' · 랜딩(hero 보조 버튼, FAQ 절 끝 링크).

- `how-to` — 파일 선택 → 개요(타입·타깃) → 품질·발견 → 변수별·관계 → 전처리 → 타깃 → 저장·공유·지우기. 결과 탭 순서와 같게 둠
- `faq` — 랜딩·도구 화면 문답 + 일반 문답을 주제별 `<h2>` 로 묶음. **절 제목에 "자주 묻는 질문"을 쓰지 않음** — `build_seo.mjs extractFaq()` 는 그 제목에서만 FAQPage 를 만들고, 같은 문답이 랜딩·도구 화면에 이미 구조화 데이터로 있어 중복 마크업이 됨

### 3.8 `/pages/sitemap`

사람이 읽는 HTML 사이트맵. `js/app/menu.js`가 읽는 것과 같은 원천(`data/guide-nav.json`이 아니라 `scripts/build_guides.mjs`의 `report` — 빌드 시점 메모리 값)에서 해설·사례 목록을 만들어 손으로 옮겨 적지 않음. 새 글이 추가되면 다음 `npm run build`에서 자동으로 반영됨. `noindex, follow` + sitemap.xml 제외(§3.7과 같은 이유 — 탐색 전용 페이지는 색인 가치가 없음) + CTA 밴드 없음(정책 페이지와 동일).
...
| **C** 결과 | 8섹션 **탭 전환** | 개요 / 품질 / 발견 / 변수별 / 관계 / 전처리 / 타깃 / 차원축소 + `결과 내보내기`·`결과 지우기`·`새 파일` |
| **D** 오류 | 오류 안내 | 유형별 안내 — 인코딩 감지 실패(**인코딩 수동 선택 제시**) / 크기 초과 / 파싱 불가(문제 행 위치) / 스키마 불일치(문제 필드) |

**전처리 탭**([`TODO.md` T7](./TODO.md)): 발견 목록의 "무엇을 하면 되는지"를 실행 가능한 조치로 옮기는 자리임. 발견 카드의 `조치 담기`가 이 탭의 레시피로 이어지고, 적용하면 Before/After 비교와 정제 CSV 내려받기가 나옴. **기본 레시피는 비어 있음** — 도구가 알아서 고치지 않는다는 원칙이 화면에서도 보여야 하므로 제안은 전부 꺼진 채로 시작하고 각 조치의 대가를 함께 적음. 불러오거나 복원한 결과에는 원본 행이 없어 이 탭이 비활성됨.

**타깃 탭**(docs/TODO.md T7 2단계, UC-21): 개요 탭 상단의 "타깃 열" select에서 열을 고르면 전체를 다시 계산하고 이 탭이 채워짐. 타깃 타입에 따라 분기함 — 회귀(수치형)는 타깃 분포·왜도 안내·피처별 `|Pearson|` 상관 랭킹을, 분류(범주형·불리언)는 클래스 분포와 클래스별 수치 열 요약표를 보여줌. 타깃 고유값이 많으면(고카디널리티) 전체 분포 대신 상위 값만 보이고 클래스 불균형 판정은 제공하지 않음(`data-model.md §3.3`). 미지정 시에는 안내 문구만 표시함.

**차원축소 탭**([`TODO.md` T11](./TODO.md)): 히트맵이 두 열씩만 다루는 것을 수치형 열 전체 관점으로 옮긴 자리임. 표준화한 뒤 상관행렬 기준으로 주성분을 구해 스크리 막대·누적 설명 분산표·성분별 로딩 표를 보여줌. 산출 조건([`rules.md §6.2`](./rules.md))에 미달하면 조건을 밝힌 안내 문구만 표시함. **주성분 점수(행 단위 값)는 담지도 그리지도 않음** — 여기 있는 것은 전부 집계값임([`data-model.md §3.9`](./data-model.md)). 축약의 대가(열 이름이 갖던 의미를 잃음)를 화면 상단에 함께 적고 해설 R2(`multicollinearity`)로 링크함.

**탭 전환을 택한 이유**: 여러 섹션을 한 화면에 쏟으면 G2(정보 과부하)를 그대로 반복함. 사전조사가 지적한 "50개 변수 → 히스토그램 50개" 문제를 화면 구조 차원에서 막음. Phase 2의 STEP 가이드(UC-20)는 이 탭 위에 진행 모드로 얹으며 별도 페이지를 만들지 않음.

**관계 탭 상단에는 범주형↔수치형 관계 안내가 고정으로 붙음** — 범주형(또는 불리언)과 수치형이 함께 있을 때만 표시하고, 해설 R4 로 링크함. 원래 `F-MIXED-RELATION` 발견이었으나 항상 발화해 폐지하고 이 자리로 옮겼음 ([`rules.md §6.3`](./rules.md)).

**관계 탭은 이어서 세 구획을 더 담음**(수치형 열이 2개 미만이라 상관을 건너뛴 경우에도 아래 두 구획은 독립적으로 표시됨):
1. **상관 히트맵·산점도** — 기존 UC-08 그대로. 수치형 열이 2개 미만이면 이 구획만 생략.
2. **범주형 연관성** — Cramér's V 히트맵. 카디널리티가 낮은 범주형 열이 2개 이상일 때만 표시.
3. **다중 컬럼 관계** — 그룹상관(범주형 기준 소그리드 산점도) + 편상관(통제 전후 비교 텍스트). 상관 절댓값 상위 후보에서 찾은 것만 표시하며, 없으면 구획째 생략.

두 구획 모두 구버전 캐시(`schemaVersion` 1.2 이하 — `associations`·`interactions` 필드가 없음)를 안전하게 건너뜀.

**각 탭의 표시 개수 상한은 [`rules.md §4`](./rules.md)가 단일 원천임** — 발견 목록 기본 15건, 변수별 초기 차트 8열, 산점도 6쌍, 히트맵 20열, 다중 컬럼 관계 후보 3건, 범주형 연관성 히트맵 20열, 주성분 10개. 수치를 여기 옮겨 적지 않고 참조함.
...
| 요소 | 내용 |
|---|---|
| **헤더** | 로고 + 네비(도구 / 해설 / 사례 / 용어집 / 문의). 빌드 도구가 없으므로 페이지마다 마크업을 복사하고 수정 시 함께 갱신함 (`weareants` 규약) |
| **전역 메뉴** | 우상단 고정 `☰` + 드롭다운. `BDAnalyzer/js/menu.js`가 import만으로 버튼·패널을 스스로 생성해 `body`에 붙이는 방식이므로 **그 패턴을 재사용**함. 각 HTML에는 `<script type="module" src="/js/app/menu.js">` 한 줄만 추가. '해설' 항목에는 접이식 하위목록을 둠 — 목록은 `data/guide-nav.json`(build_guides 산출물)이며 처음 펼칠 때 한 번만 받음 |
| **푸터** | 정책 4종 링크. `noindex`이지만 `follow`이므로 크롤러·심사자 도달 경로로 유지함 |
| **브레드크럼** | 해설·사례 하위 페이지. BreadcrumbList JSON-LD 동반 |
| **CTA** | `내 데이터로 확인하기` — 모든 해설·사례 페이지 하단 고정. 짙은 녹색 라운드 카드(House Green)에 흰 알약 버튼 |
| **Feature band** | 랜딩 중간의 전면 폭 짙은 녹색 색블록(`.band`). 크림 → 흰 카드 → 다크 밴드 → 크림 → 다크 푸터의 교대 리듬을 만듦 ([`DESIGN.md` §4](DESIGN.md)). 다크 면은 이 둘(하단 CTA·밴드)과 푸터뿐이라 광고와 시각적으로 구분됨 |
| **플로팅 CTA** | 우하단 고정 56px 원형 도구 진입점. `js/app/float-cta.js` 가 import 만으로 생성하므로 각 HTML 에는 스크립트 한 줄만 둠(메뉴와 같은 패턴). **도구 화면에서는 붙지 않음** |
