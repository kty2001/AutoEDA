...
| 경로 | 타입 | 필수 | 설명 |
|---|---|:---:|---|
| `schemaVersion` | string | ○ | `major.minor`. UC-10이 검증. major 불일치는 거부. minor는 선택 필드 추가에만 올림(1.0 → 1.1: `histogram.density`, 1.2: `dataset.recipe`, 1.3: `columns[].classStats`, 1.4: `associations`·`interactions`) |
| `dataset` | object | ○ | §3.2 |
